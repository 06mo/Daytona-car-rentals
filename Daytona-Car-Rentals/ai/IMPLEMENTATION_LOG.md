...
| 2026-03-27 | Codex | `/api/vehicles/availability` date validation fix | ✅ Reviewed and signed off. Added `Number.isNaN` guard after `new Date(startDate)` / `new Date(endDate)` — invalid/malformed date strings now return 400 instead of falling through to Firestore Timestamp serialization (which caused Vercel 500s). Added `parsedEndDate <= parsedStartDate` inverted-range check returning 400. Root cause: production 500s were likely bad input strings reaching `new Date(...)` and exploding during Firestore write. No logic changes to the availability query itself. Pre-existing: bulk-path Timestamp vs Date comparison on line 63 (`booking.endDate > parsedStartDate`) is consistent with existing `isVehicleAvailable` behaviour — not introduced by this patch. |
